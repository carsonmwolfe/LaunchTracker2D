...
class LaunchPadDisplay:
    def __init__(self, root):
        ...
        self.aircraft_start_time = time.time() * 1000  # Convert to milliseconds
        
        # Tower blinking lights
        self.tower_lights_on = True
    ...
    def animate_smoke(self):
        ...
        # Only show smoke if there's a rocket on the pad AND it's not currently launching
        rocket_items = self.canvas.find_withtag('rocket')
        is_launching = self.launch_animator and self.launch_animator.is_launching
        
        if len(rocket_items) > 0 and self.launch_data and not is_launching:
            draw_smoke_effect(self.canvas, self.smoke_frame, self.launch_data)
            self.smoke_frame += 1
        else:
            # Clear smoke when no rocket on pad or during launch
    ...
    def spawn_cars(self):
        """Create initial cars - simple left to right movement."""
        car_colors = ['#3a7bc8', '#d44444', '#f5f5f5', '#2a2a2a', '#ffd93d']
        
        for i in range(3):
            start_x = -50 - (i * 150)  # Stagger starting positions
            y = 426  # Center of new road position (road is at y=420)
            
            speed = random.uniform(0.5, 1.0)
            color = random.choice(car_colors)
            car_ids = draw_car(self.canvas, start_x, y, color)
    ...
    def animate_cars(self):
        """Animate cars driving left to right."""
        for car in self.cars:
            ...
            # Check if car is off screen, respawn on left
            if car['x'] > 850:
                for car_id in car['ids']:
                    self.canvas.delete(car_id)
                
                # Respawn off screen to the left
                start_x = random.randint(-200, -50)
                y = 426  # Center of road
                speed = random.uniform(0.5, 1.0)
                car['ids'] = draw_car(self.canvas, start_x, y, car['color'])
                car['speed'] = speed
                car['x'] = start_x
                car['y'] = y
    ...
    def animate_tower_lights(self):
        """Animate blinking white lights on launch tower - flash for 1 second every 5 seconds."""
        # Get all tower light elements
        light_items = self.canvas.find_withtag('tower_light')
        
        # Turn lights ON (bright white)
        for item in light_items:
            self.canvas.itemconfig(item, fill='#ffffff', outline='#eeeeee')
        
        # Schedule turning lights off after 1 second
        self.root.after(1000, self.turn_off_tower_lights)
        
        # Schedule next flash in 5 seconds
        self.root.after(5000, self.animate_tower_lights)
    
    def turn_off_tower_lights(self):
        """Turn off the tower lights."""
        light_items = self.canvas.find_withtag('tower_light')
        
        for item in light_items:
            self.canvas.itemconfig(item, fill='#3a3a3a', outline='#2a2a2a')
    
    def animate_aircraft(self):
        """Animate T-38 aircraft flyby."""
        current_time = time.time() * 1000 - self.aircraft_start_time
        
        # Check if it's time to start a new flyby
        if self.aircraft.should_start_flyby(current_time):
            self.aircraft.start_flyby()
            self.aircraft.last_update_time = current_time
        
        # Update aircraft position if active
        if self.aircraft.active:
            delta = current_time - self.aircraft.last_update_time
            self.aircraft.update(delta)
            self.aircraft.last_update_time = current_time
        
        # Continue animation loop (every 50ms for smooth movement)
        self.root.after(50, self.animate_aircraft)
